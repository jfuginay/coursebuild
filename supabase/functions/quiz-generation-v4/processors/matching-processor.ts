...
export const generateMatchingQuestion = async (plan: QuestionPlan): Promise<MatchingQuestion> => {
  try {
    ...
    const contextualPrompt = buildMatchingContextualPrompt(plan);
    ...
    // Convert to our MatchingQuestion interface
    const matchingQuestion: MatchingQuestion = {
      ...
      relationship_analysis: questionData.relationship_analysis || {},
      relationship_type: questionData.relationship_analysis?.relationship_type || 'concept-definition'
    };
    
    console.log(`✅ Matching generated successfully: ${plan.question_id}`);
    console.log(`   🎯 Question: ${questionData.question.substring(0, 60)}...`);
    console.log(`   🔗 Pairs: ${questionData.matching_pairs.length}`);
    console.log(`   📋 Relationship Type: ${questionData.relationship_analysis?.relationship_type || 'Not specified'}`);
    console.log(`   📝 Sample Pair: "${questionData.matching_pairs[0]?.left}" → "${questionData.matching_pairs[0]?.right}"`);
    console.log(`   🤖 Provider Used: ${response.provider || 'unknown'}`);
    
    return matchingQuestion;
    
  } catch (error: unknown) {
    console.error(`❌ Matching generation failed for ${plan.question_id}:`, error);
    throw new QuestionGenerationError(
      `Matching generation failed: ${error instanceof Error ? error.message : String(error)}`,
      plan.question_id,
      { plan, stage: 'matching_generation' }
    );
  }
};
...
const buildMatchingContextualPrompt = (plan: QuestionPlan): string => {
  return `${MATCHING_DETAILED_PROMPT}
...
1. Tests the specified learning objective through meaningful relationship understanding at the ${plan.bloom_level} level
2. Incorporates the key concepts through authentic relationship connections
3. Uses relationship patterns that reveal genuine understanding vs. guessing
4. Provides educational explanations that reinforce understanding of WHY items connect

Focus on creating relationship connections that develop understanding of important patterns and principles, not just memorized associations.`;
};
...
export const matchingProcessorConfig = {
  ...
  processorName: 'Matching Processor v4.0',
  stage: 2,
