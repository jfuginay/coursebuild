...
export const MCQ_SCHEMA_BASE = {
  ...
  properties: {
    ...
    optimal_timestamp: {
      type: "number",
      description: "The optimal timestamp (in seconds) for this question to appear, after all relevant concepts have been explained"
    },
...
export const SEQUENCING_SCHEMA_BASE = {
  ...
  properties: {
    ...
    sequence_items: {
      ...
      minItems: 3,
      maxItems: 6,
      description: "3-6 items that need to be ordered in correct sequence (as strings)"
    },
    explanation: { 
      type: "string", 
...
const validateSequencingResponse = (data: any): boolean => {
  ...
  for (let i = 0; i < data.sequence_items.length; i++) {
    if (typeof data.sequence_items[i] !== 'string') {
      console.error(`❌ Sequencing item ${i} must be a string`);
      return false;
    }
  }
