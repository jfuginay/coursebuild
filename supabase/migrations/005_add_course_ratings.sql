...
CREATE INDEX idx_user_course_ratings_course_id ON user_course_ratings(course_id);
CREATE INDEX idx_user_course_ratings_rating ON user_course_ratings(rating);
CREATE INDEX idx_user_course_ratings_user_id ON user_course_ratings(user_id);
CREATE INDEX idx_user_course_ratings_created_at ON user_course_ratings(created_at);
...
CREATE TRIGGER rating_stats_refresh_trigger
    AFTER INSERT OR UPDATE OR DELETE ON user_course_ratings
    FOR EACH STATEMENT
...
CREATE TRIGGER auto_engagement_score_trigger
    BEFORE INSERT OR UPDATE ON user_course_ratings
    FOR EACH ROW
...
COMMENT ON TABLE user_course_ratings IS 'User ratings for courses (1-5 stars) with engagement tracking';
